...
export const UserRegister = async (req, res, next) => {
  try {
    ...
    const token = jwt.sign({ id: createduser._id }, "harsh", {
...
export const UserLogin = async (req, res, next) => {
  try {
    ...
    const token = jwt.sign({ id: existingUser._id }, "harsh", {
      expiresIn: "9999 years",
    });
...
export const addToCart = async (req, res, next) => {
  try {
    ...
    console.log(userJWT)
    const parsedData = jwt.verify(userJWT,"harsh")
...
export const getAllCartItems = async (req, res, next) => {
  try {
    ...
    const user = await User.findById(parsedData.id).populate({
      path: "cart.product",
      model: "Products",
    });
...
export const getUserFavourites = async (req, res, next) => {
  try {
    ...
    const user = await User.findById(parsedData.id).populate("favourites").exec();
